...
///////////////
var rtse_dl_win;
var dl_fname="rtse.txt";
var dl_data="";

function openDownloadDialogWindow()
{
  rtse_dl_win = window.open("dl_dialog.html", "DL_Dialog", "width=800,height=600, scrollbars=1,menubar=1");
  rtse_dl_win.focus(); 
  return rtse_dl_win;
}
...
function getDisplayedRtcs(flag){
  var res = new Array();
...
function exitAll(flag){
  var rtcs = getDisplayedRtcs(1);

  if(flag){ rtcs = getAllRtcsFromNsView(); }
  for(var i=rtcs.length; i > 0;i--){
...
function requestRestoreComponent(fullpath, x, y)
{
  var data=new Array();
  data['cmd'] = "rtcat";
  ...
  if(!component_list[fullpath]){
    displayInfoMessage("<font color=\"blue\">Request Component Info..., Please wait for a while.</font>");
    postRequest(rtsh_com, data, restoreRtc, [x, y]);

  }else{
    var comp = component_list[fullpath];
    if(comp.display == 'none'){
      comp.showItem();
    }
    setObjectPos(comp, x, y);
  }
}
...
function requestConnectionInfo(obj1, obj2, conn)
{
  var data=new Array();
  ...
  postRequest(rtsh_com, data, finishConnectionInfo, conn);
  selectedItems=[];
}
...
function finishConnectionInfo(s, conn){
  conn.obj = JSON.parse(s);
  mkConnectors('Connector');
  refreshAll();
}
    ...
    if(cmp.Ports){
      for(var i=0; i<cmp.Ports.length;i++){
        ...
        for(var j=0;j<p.Connections.length;j++){
          createConnectors(Rtc.getFullName()+":"+p.Name,
			  p.Connections[j].DestPorts,
			  false );
...
function restoreRtc(s, pos){
  try{
    var cmp = JSON.parse(s);
    var Rtc = addComponent(pos[0], pos[1], cmp, 1);
    ...
    displayInfoMessage("Error in restoreRtc."+e);
  }
}
...
function findRtcById(id){
  var i;
  for(i=0; i<Rtc_table.length;i++){
    if(Rtc_table[i].getId() == id){
      return Rtc_table[i];
    }
  }
  return null;
...
function updateConnector(name1, name2, flag){
  ...
  if( start_obj && end_obj){
    conn = updateConnectorFromItem(start_obj, end_obj)
  }else{
...
function updateConnectorFromItem(port1, port2){
  ...
  if(!connector){
    ...
    /// Request Connection
    requestConnectionInfo(port1, port2, connector)
  }
...
function drawContent(){
  var txt = getSvgCanvas().getSvgContent();
  var ele = document.getElementById('profile');
  var content ="";

  //content +="<input type=\"button\" onClick=\"handleWriteToFile('rtsprofile');\" value=\"Save\"">";
  content +="<input type=\"text\" onClick=\"changeDownloadFilename();\" value=\"rtse.svg\" id=\"rtsname\">";
